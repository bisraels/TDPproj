...
clc
%%
% AUHTOR: Claire Albrecht (with reference to Brett Israels file: dwell2TDP_fromBrett)
% CREATED: July 2019
%
% PURPOSE: Takes dwell functions of HMM fits (made in pathdwellredo) and turn it into TDPs.
% Using Brett's file (dwell2tdp_fromBrett) as a template to understand how the program works
%
% MODIFICATIONS:
%   July 2019   Find local maxima on TDP plots and label maxima with dots
%               Determine which rates exist and which are zero from the TDP
%               plot
%               Created functions out of this script: locMax_finder,
%               rates_obs_finder
%
% INPUT: dwell files
% Dwell file format:
...
%
% OUTPUT: TDP plot (fig1) and matrix of existing rates (rates_obs)
%
% NOTE: This is the second version of this file, I got lost in the weeds
% the first time and wanted to start fresh.
% NOTE 2: This is now the third version, dwellTDP_2 does everything up to
...
for molID = 1:numFiles
    ...
    dwellFileName = fileNames(molID).name;        % Obtain the name of the dwell file
    ...
    scanID = dwellFileName(1:length(scanIDFormat)); % Define scanID as the number of files in the set of molID's from above
    
    
    if fileNames(molID).bytes > 0                 % If the 
...
%xlim and ylim
 shading interp % this emliminates the black lines from the gridding
